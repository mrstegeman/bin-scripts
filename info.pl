...
## what values to display. Use "OS Kernel DE Cursor WM win_theme Theme Font Icons" ##
my $display = "OS Kernel Cursor WM Win_theme Theme Icons Font Background";
...
if ( $display =~ m/[Theme, Icons, Font, Background]/) {
  switch($DE) {
    ...
    case "KDE" { 
      ...
      my @files = ("$ENV{HOME}/.gtkrc-2.0", "$ENV{HOME}/.gtkrc.mine", "$ENV{HOME}/.config/nitrogen/bg-saved.cfg", "$ENV{HOME}/.Xdefaults",);
